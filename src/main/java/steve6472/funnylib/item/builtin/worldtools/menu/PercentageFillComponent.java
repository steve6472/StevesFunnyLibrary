 ...
 */
public class PercentageFillComponent extends Menu
{
	...
	public PercentageFillComponent(FillerMenu fillerMenu)
	{
	...
	protected void setup()
	{
		...
		for (int i = 0; i < 3; i++)
		{
			...
			{
				if (i == 2 && j == 2) continue;
				String material = data.protectedData().getOrCreateCompound("percentage").getString("slot_" + (i + j * 3), Material.AIR.toString());
		...
		setSlot(2, 2,
			airScroll = new ClickNumberScroller(
				...
				() -> {
					...
					int maxAir = 9;
					int slots = (int) itemData.protectedData().getOrCreateCompound("percentage").getKeys().stream().filter(p -> p.startsWith("slot_")).count();
					return maxAir - slots;
				},
				() -> {
					ItemNBT itemData = fillerMenu.nbtFromPlayersHandOrStack(fillerMenu.itemStack);
					...
					return itemData.protectedData().getOrCreateCompound("percentage").getInt("air_count", 0);
				},
				(value) -> {
					ItemNBT itemData = fillerMenu.nbtFromPlayersHandOrStack(fillerMenu.itemStack);
					if (itemData == null) return;

					NBT advanced = itemData.protectedData().getOrCreateCompound("percentage");
					advanced.setInt("air_count", value);
					itemData.protectedData().setCompound("percentage", advanced);
					itemData.save();
				}
			)
		);
		...
		setSlot(x, y,
			new ItemSwapSlot(a, true, false)
				...
				.onClear(() ->
				{
					ItemNBT itemData = fillerMenu.nbtFromPlayersHandOrStack(null);
					if (itemData == null) return;
					if (!itemData.protectedData().hasCompound("percentage")) return;
					NBT advanced = itemData.protectedData().getCompound("percentage");
					advanced.remove(id);
					itemData.protectedData().setCompound("percentage", advanced);
					itemData.save();
	...
	{
		return ItemStackBuilder.quick(Material.BUCKET, "Air Percentage");
	}
}
